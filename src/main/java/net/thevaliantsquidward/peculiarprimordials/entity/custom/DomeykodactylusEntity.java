...
public class DomeykodactylusEntity extends EntityBaseDinosaurAnimal implements GeoEntity, NeutralMob, IBookEntity {
    ...
    public DomeykodactylusEntity(EntityType<? extends EntityBaseDinosaurAnimal> entityType, Level level) {
...
//variant code
    ...
    protected <E extends DomeykodactylusEntity> PlayState Controller(AnimationState<E> event) {
        if (this.isFromBook()) {
    ...
    protected <E extends DomeykodactylusEntity> PlayState attackController(AnimationState<E> event) {
    ...
    static {
        FLYING = SynchedEntityData.defineId( DomeykodactylusEntity.class, EntityDataSerializers.BOOLEAN);
        CROPS_POLLINATED = SynchedEntityData.defineId( DomeykodactylusEntity.class, EntityDataSerializers.INT);
        FROM_BOOK = SynchedEntityData.defineId( DomeykodactylusEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    private class AIFlyIdle extends Goal {
        ...
        public boolean canUse() {
            if (! DomeykodactylusEntity.this.isVehicle() && ( DomeykodactylusEntity.this.getTarget() == null || ! DomeykodactylusEntity.this.getTarget().isAlive()) && ! DomeykodactylusEntity.this.isPassenger()) {
                if ( DomeykodactylusEntity.this.getRandom().nextInt(45) != 0 && ! DomeykodactylusEntity.this.isFlying()) {
                    return false;
                } else {
        ...
        public void tick() {
             DomeykodactylusEntity.this.getMoveControl().setWantedPosition(this.x, this.y, this.z, 1.0);
            if ( DomeykodactylusEntity.this.isFlying() &&  DomeykodactylusEntity.this.onGround() &&  DomeykodactylusEntity.this.timeFlying > 10) {
                 DomeykodactylusEntity.this.setFlying(false);
            }
        ...
        protected Vec3 getPosition() {
            Vec3 vector3d =  DomeykodactylusEntity.this.position();
            return  DomeykodactylusEntity.this.timeFlying >= 200 && ! DomeykodactylusEntity.this.isOverWaterOrVoid() ?  DomeykodactylusEntity.this.getBlockGrounding(vector3d) :  DomeykodactylusEntity.this.getBlockInViewAway(vector3d, 0.0F);
        }

        public boolean canContinueToUse() {
            return  DomeykodactylusEntity.this.isFlying() &&  DomeykodactylusEntity.this.distanceToSqr(this.x, this.y, this.z) > 5.0;
        }

        public void start() {
             DomeykodactylusEntity.this.setFlying(true);
             DomeykodactylusEntity.this.getMoveControl().setWantedPosition(this.x, this.y, this.z, 1.0);
        }

        public void stop() {
             DomeykodactylusEntity.this.getNavigation().stop();
            this.x = 0.0;
            this.y = 0.0;
            this.z = 0.0;
            super.stop();
