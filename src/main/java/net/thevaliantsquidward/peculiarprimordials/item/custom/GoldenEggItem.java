...
public class GoldenEggItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        if (!pLevel.isClientSide) {
            GoldenGiganhingaEggEntity egg = new GoldenGiganhingaEggEntity(pLevel, pPlayer);
            egg.setItem(itemstack);
            egg.shootFromRotation(pPlayer, pPlayer.getXRot(), pPlayer.getYRot(), 0.0F, 1.5F, 1.0F);
